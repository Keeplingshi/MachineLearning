...
class TestMain { // 主函数
	...
	public static void testICTCLAS_ParagraphProcess(String sInput) {
		try {
			...
			// 导入用户字典
			int nCount = 0;
			String usrdir = Global.USER_EXTRA; // 用户字典路径
			byte[] usrdirb = usrdir.getBytes();// 将string转化为byte类型
			// 导入用户字典,返回导入用户词语个数第一个参数为用户字典路径，第二个参数为用户字典的编码类型
			nCount = testICTCLAS50.ICTCLAS_ImportUserDictFile(usrdirb, 1);
	...
	public static void testICTCLAS_FileProcess() {
		try {
			...
			nCount = testICTCLAS50.ICTCLAS_ImportUserDictFile(usrdirb, 1);// 导入用户字典,返回导入用户词语个数
			System.out.println("导入用户词个数" + nCount);
			nCount = 0;
