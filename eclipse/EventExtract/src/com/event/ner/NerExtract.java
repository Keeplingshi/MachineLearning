package com.event.ner;

import java.util.ArrayList;
import java.util.List;

/**
 * 创建时间：2015年9月20日 项目名称：Java_EventDetection_News
 * 
 * @author chenbin
 * @version 1.0
 * @since JDK 1.6 文件名称：NerExtract.java 系统信息：Win7 类说明： 功能描述：
 *        调用NerTools的三个方法seg、posTagging、ner，并且根据词性标注结果识别出text中的名词与动词
 */
public class NerExtract {
	 ...
	 * @param text 待分词的字符串
	 */
	public List<Pair<String, String>> nerResult(String text) {
	 ...
	 * @param text 待分词的字符串
	 */
	public List<Pair<String, String>> tagResult(String text) {
	 ...
	 * @param text 字符串
	 * @return re 输出字符串中的名词与动词  -1:名词 1:动词 0:其他
	 * 				example:[在=0, 冲绳=-1, 大战=-1, 当中=0, ，=0, 有=1, 一场=0,战斗=1]
	 */
	public List<Pair<String, Integer>> getN_V(String text) {
		List<Pair<String, String>> list = nertools.posTagging(text);
		List<Pair<String, Integer>> re = new ArrayList<Pair<String, Integer>>();
		/*
		 * -1:名词 1:动词 0:其他
		 */
		for (Pair<String, String> pairs : list) {
			...
			} else if (pairs.second.matches("v.*")) {
				re.add(new Pair<String, Integer>(pairs.first, 1));
			} else {
	...
	public static void main(String[] args) {
		...
		System.out.println(ne.getN_V(text));//识别出text中的名词与动词
