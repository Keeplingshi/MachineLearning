...
public class SentDetectorWrapper
{
	...
	public static SentDetectorWrapper getSentDetector()
			throws InvalidFormatException, IOException
	{
		if (sentDetectorWrapper == null)
		{
			sentDetectorWrapper = new SentDetectorWrapper(new File(
					"data/en-sent.bin"));
		}
	...
	SentDetectorWrapper(File model_File) throws InvalidFormatException,
			IOException
	{
	...
	public util.Span[] detectPosMonocase(String text)
	{
	 ...
	 * @return
	 */
	public util.Span[] detectPos(String text)
	{
		ArrayList<util.Span> ret = new ArrayList<util.Span>();
		...
		return ret.toArray(new util.Span[ret.size()]);
	}

	private void segBuffer(ArrayList<util.Span> ret, String buffer,
			int offset)
	{
		...
		for (int i = 0; i < spanBuffer.length; i++)
		{
			...
			util.Span adjustedSpan = new util.Span(span.getStart()
					+ offset, span.getEnd() - 1 + offset);
			...
			IOException
	{
		...
		util.Span[] spans = SentDetectorWrapper.getSentDetector()
				.detectPosMonocase(text);

		for (util.Span span : spans)
		{
