...
/**
 * 创建时间：2015年10月12日 项目名称：Java_EventDetection_News
 * 
 * @author fangqian
 * @version 1.0
 * @since JDK 1.6 文件名称：ReadTable.java 系统信息：Win7 类说明：
 *        功能描述：1、读取event.xlsx(已经标好触发词以及其类别的标题),2、统计每一类出现的触发词以及个数，3、统计每个触发词所对应的类别
 *        ，以及相应的标题
 */
public class ReadTable {
	...
	/**
	 * 读取event.xlsx 统计每一类的触发词以及触发词所对应的标题ID到twWordNumofClass,
	 * 如“[{"举行",[1,3,4,6]},{"访问",[3,4,6,7]}]” 统计触发词的数目到triggerWordNum
	 * 如“[“举行”,“100”]” 统计每一类对应的标题个数到docNumOfClass 如["第一类","100"]
	 */
	public void read() throws IOException {
		
		// 读取event.xlsx
		InputStream is = new FileInputStream(new File("E:/Github/MachineLearning/eclipse/Crawler_EventDetection/src/event.xlsx"));
		...
		for (int i = 1; i <= rsRow; i++)
		{
			...
			if (xr != null)
			{
				...
				{
					key1 = xr.getCell(1).getRawValue();// 22类
				}
	...
	/**
	 * 将twWordNumofClass、docNumOfClass写入到文件summarize.txt中,
	 * 并且对twWordNumofClass中的触发词出现频率进行排序
	 * 
	 * @param frontId
	 *            指定写入文件的每一类触发词个数
	 * @return 写入文件
	...
	public void eventSummarize(int frontId) {
		...
		try {
			...
			for (classId = 1; classId <= 22; classId++) {
				...
				List<Map.Entry<String, List<Integer>>> temp = new ArrayList<>(
						twWordNumofClass[classId - 1].myMap.entrySet());
			...
			try {
				...
				for (int j = 0; j < 22; j++)/* 统计触发词在每个类中出现的次数 */
				{
					if (twWordNumofClass[j].myMap.containsKey(triggerName)) {
						...
						trigger.add(new Pair<String, List<Integer>>(String.valueOf(j + 1),
								twWordNumofClass[j].myMap.get(triggerName)));
					}
				}
				...
				InputStream is = new FileInputStream(
						new File("E:/Github/MachineLearning/eclipse/Crawler_EventDetection/src/event.xlsx"));
				readwb = new XSSFWorkbook(is);
